...
Account::Account(const std::string &username, const std::string &password, const std::string &phoneNumber) 
...
bool Account::updateInformation() {
    displayCol();
    std::string select;
    std::cout << "You can change password, phone number.\n"
              << "Enter your choice (string): ";
    std::cin >> select;
    if (toLowerCase(select) == "password") {
        std::string newPassWord = inputPassword("Enter new PassWord: ");
        if (confirmPassWord()) {
            passWord = newPassWord;
            std::cout << "Change password successful!\n";
            return true;
        } else {
            std::cout << "Invalid password confirmation\n";
        }
    } else if (toLowerCase(select) == "phonenumber" ||
               toLowerCase(select) == "phone number" ||
               toLowerCase(select) == "numberphone" ||
               toLowerCase(select) == "number phone") {
        std::string newPhoneNumber =
            inputPhoneNumber("Enter new Phone Number: ");

        if (confirmPassWord()) {
            this->phoneNumber = newPhoneNumber;
            std::cout << "Change Phone Number successful!\n";
            return true;
        } else {
            std::cout << "Invalid password confirmation\n";
        }
...
bool Account::confirmPassWord() {
    std::cout << "Please enter your password (confirm): ";
    std::string inputPass = inputPassword("");
