...
void UserMenu(Movies &movies, ShowTimes showtimes, Accounts &accounts,
  ...
  auto show_booked_tickets_button = Button("Show Ticket history", [&] {
    ShowTicketInfoForAccount(account, accounts.getAccounts(),
                             tickets.getTickets(), movies.getMovies(),
  ...
  auto renderer = Renderer(container, [&] {
    return vbox({
               ...
               separator(),
               book_ticket_button->Render() | center | size(WIDTH, EQUAL, 50),
               show_movies_button->Render() | center | size(WIDTH, EQUAL, 50),
...
void AdminMenu(Movies &movies, ShowTimes showtimes, Accounts &accounts,
  ...
  auto renderer = Renderer(container, [&] {
    return vbox({
               ...
               update_account_button->Render() | center |
                   size(WIDTH, EQUAL, 50),
               statistiic_button->Render() | center | size(WIDTH, EQUAL, 50),
               logout_button->Render() | center | size(WIDTH, EQUAL, 50),
           }) |
           border | size(WIDTH, EQUAL, 100) | center;
  });
...
int main() {
  ...
  while (true) {
    ...
    clear();
    std::cout << account.getRole() << std::endl;
    system("pause");
