...
void addMovie(Movies &movies) {
    ...
    auto confirm_button = Button("confirm", [&] {
        message = "";
        if (title.empty() || description.empty() || genre.empty() || director.empty() ||
            cast.empty() || duration.empty() || releaseDate.empty()) {
...
void addShowTime(const Movie &movie, ShowTimes &showtimes) {
    ...
    auto component =
            Container::Vertical({
                hall_id_input, show_date_input, start_time_input,
...
void UpdateAccount(Accounts &accounts, Account account) {
    ...
    auto component = Container::Vertical({
        input_password, input_phone_number,
...
int selectMovieForAddShowTime(const Vector<Movie> &movie){
    Vector<Movie> movies = movie;
    using namespace ftxui;
    std::string movieId;
    std::string search_query;
    std::string search_message;
...
int selectShowTime(Vector<Movie> &movies, Vector<ShowTime> &showtimes) {
    ...
    auto renderer = Renderer(container, [&] {
        return vbox({
                   ...
                   center,
                   hbox({
                       showtime_input->Render() | size(WIDTH, EQUAL, 25) | center,
