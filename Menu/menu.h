        ...
        if (hallId.empty() && showDate.empty() && startTime.empty()) {
            ...
            if (validInteger(hallId)) {
                ...
                if (hallID < 4 && hallID > 0) {
                    ...
                    ShowTime newShowTime(movieId, hallID, showDate, startTime,
                                         endTime);
                    message = "Step 2";
...
void UpdateAccount(Accounts &accounts, Account account) {
    ...
    Component input_phone_number =
        Input(&phoneNumber, account.getPhoneNumber());
    ...
    for (const auto &ticket : tickets) {
        if (account.getUserId() == ticket.getUserId()) {
            // Check if the showtime exists for the ticket
                ...
                showtime_rows.push_back(separator());
                showtime_rows.push_back(hbox(
                    text(std::to_string(ticket.getTicketId())) |
                        size(WIDTH, EQUAL, 15),
                    separator(),
                    paragraph(movie.getTitle()) | size(WIDTH, EQUAL, 20),
                    separator(),
...
ftxui::Element DisplayCheckedTicket(const Movie &movie,
    ...
    for (int ticketId : isSelected) {
        ...
        ticket_rows.push_back(hbox(
            ...
            text(showtime.getStartTime().toString()) | size(WIDTH, EQUAL, 12),
            separator(),
            text(std::to_string(ticketId)) | size(WIDTH, EQUAL, 12),
            separator(),
...
bool Step3_SelectSeat(const Movie &movie, const ShowTime &showtime,
    ...
    auto book_button = Button("Book Selected Seats", [&] {
        if (!selectedSeats.empty()) {
            ...
            if (Step4_DisplayCheckedTicket(movie, showtime, account,
...
//     if (tickets.empty()) {
//         std::cout << "No tickets sold." << std::endl;
//         return;
//     }
...
//     int totalTickets = 0;
//     int totalRevenue = 0;
//     int movieSales[100] = {0};  // Giả sử có tối đa 100 bộ phim, bạn có thể điều
//                                 // chỉnh kích thước này

//     // Duyệt qua tất cả vé
//     for (const auto &ticket : tickets) {
//         totalTickets++;
//         totalRevenue += ticket.getTicketPrice();
//         movieSales[ticket.getMovieId()]++;  // Tăng số vé cho bộ phim tương ứng
//     }

//     std::cout << "Total Tickets Sold: " << totalTickets << std::endl;
//     std::cout << "Total Revenue: $" << totalRevenue << std::endl;

//     // Tìm bộ phim phổ biến nhất
//     int mostPopularMovieId = -1;
//     int maxSales = 0;

//     for (int i = 0; i < 100; ++i) {  // Duyệt qua mảng movieSales
//         if (movieSales[i] > maxSales) {
//             maxSales = movieSales[i];
//             mostPopularMovieId = i;
//         }
//     }

//     // Hiển thị bộ phim phổ biến nhất
//     if (mostPopularMovieId != -1) {
//         for (const auto &movie : movies) {
//             if (movie.getMovieId() == mostPopularMovieId) {
//                 std::cout << "Most Popular Movie: " << movie.getTitle()
//                           << " (Tickets Sold: " << maxSales << ")" << std::endl;
//                 break;
//             }
//         }
//     }
